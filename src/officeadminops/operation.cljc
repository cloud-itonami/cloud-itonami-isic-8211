(ns officeadminops.operation
  "OperationActor -- one coordination request = one supervised actor run,
  expressed as a langgraph-clj StateGraph. The advisor (OfficeAdminAdvisor)
  is sealed into a single node (:advise); its proposal is ALWAYS routed
  through the OfficeAdminGovernor (:govern) and the rollout phase gate
  ...
  human office-administration coordinator. The approver resumes with
...
(defn build
  ...
  `officeadminops.store/Store`).
  opts:
    :advisor      -- an `officeadminops.advisor/Advisor` (default: mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; OfficeAdminAdvisor inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (advisor/-advise advisor store request)]
            {:proposal p :audit [(advisor/trace request p)]})))

      ;; OfficeAdminGovernor -- independent censor (separate system than the advisor).
      (g/add-node :govern
